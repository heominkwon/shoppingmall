...
<%
	for(int i=0 ; i <orderList.size() ; i++){
		...
		%>	
	<tr>
	<form mthod="post" name="changeform" action="changePro.jsp">
		<td align = "center" width = "100"><%=order.getO_date()%></td>
		<% OrderProductDTO opp = opd.selectOrderProduct2(order.getO_no());%>

		<td align = "center" width = "200"><%=pd.getProductName(opp.getOP_pno())%></td>
		<td align = "center" width = "200"><input type="text"  name="address" value="<%=order.getO_address() %>"></td>
		...
		<td align = "center" width = "50"><input type="submit" value="수정"></td>
		</form>
	</tr>
<%}%> 
...
<% }
    if (count > 0) {//게시글이 존재한다면
        ...
        if (startPage > 10) {    %>
        <a href="changeAddress.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        ...
        %>
        <a href="changeAddress.jsp?pageNum=<%= i %>">[<%= i %>]</a>
        
<%
        }
        if (endPage < pageCount) {  %>
         
        <a href="changeAddress.jsp?pageNum=<%= startPage + 10 
